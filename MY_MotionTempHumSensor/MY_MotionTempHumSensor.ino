...
{  
  ...
  gw.present(CHILD_ID_MOTION, S_MOTION,"motion sensor",true);  
...
{    
  ...
  //resend the Motion sensor state just in case we failed to communicate it last time it triggered.
  boolean tripped = digitalRead(DIGITAL_INPUT_SENSOR) == HIGH; 
  Serial.print("Resending Motion Sensor State: ");      
...
void readPIR() {
  ...
  boolean tripped = digitalRead(DIGITAL_INPUT_SENSOR) == HIGH; 
  Serial.print("Motion Sensor State triggered: ");      
  Serial.println(tripped);
  gw.send(msg.set(tripped?"1":"0"));  // Send tripped value to gw 
}
